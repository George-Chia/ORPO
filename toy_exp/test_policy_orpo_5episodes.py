...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="oomi-td3bc")
...
def train(args=get_args()):
     ...
     if args.rollout_policy_type == "optimistic":
          ...
          load_vae_path = f'/home/george/zyzfile/code/OfflineRL-Kit/vae_model-v2/{args.task[:-3]}'
          ...
          load_optimistic_policy_path = f'/home/george/zyzfile/code/OfflineRL-Kit/trained-optimistic-policy-v2/{args.task[:-3]}'
     ...
     x = [point[0] for point in trajectories[3260:3270]]
     y = [point[1] for point in trajectories[3260:3270]]
     ax.scatter(x, y, s=150,alpha=1,marker='^',c='#E52B50')
     # Connect the points with a line
     alphas = np.linspace(1, 0.2, len(x) - 1)
     for i in range(len(x) - 1):
          ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color='#E52B50', alpha=alphas[i])
     
     x = [point[0] for point in trajectories[50:60]]
     y = [point[1] for point in trajectories[50:60]]
     ax.scatter(x, y, s=150,alpha=1,marker='^',c='#318DE9')

     for i in range(len(x) - 1):
          ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color='#318DE9', alpha=alphas[i])
     
     x = [point[0] for point in trajectories[210:220]]
     y = [point[1] for point in trajectories[210:220]]
     ax.scatter(x, y, s=150,alpha=1,marker='^',c='#8D6AB8')
     for i in range(len(x) - 1):
          ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color='#8D6AB8', alpha=alphas[i])
              
     x = [point[0] for point in trajectories[300:310]]
     y = [point[1] for point in trajectories[300:310]]
     ax.scatter(x, y, s=150,alpha=1,marker='^',c='#00CD66')
     for i in range(len(x) - 1):
          ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color='#00CD66', alpha=alphas[i])
     
     x = [point[0] for point in trajectories[1120:1130]]
     y = [point[1] for point in trajectories[1120:1130]]
     ax.scatter(x, y, s=150,alpha=1,marker='^',c='#FFD700')
     for i in range(len(x) - 1):
          ax.plot([x[i], x[i+1]], [y[i], y[i+1]], color='#FFD700', alpha=alphas[i])
     ...
     square = patches.Rectangle((-3, -3), 6, 6, linewidth=1, edgecolor='grey', facecolor='none')
     ax.add_patch(square)
     

     x_line1 = np.linspace(-2.75, 3, 100)
     x_line2 = np.linspace(-3, 2.75, 100)
     y_line1 = -x_line1 + 0.25
     y_line2 = -x_line2 - 0.25
     ax.plot(x_line1, y_line1, color='darkblue',linewidth=2)
     ax.plot(x_line2, y_line2, color='darkblue', linewidth=2)
     ...
     ax.tick_params(axis='both', which='both', length=6, width=2, direction='out')

     plt.savefig('./trajectories_orpo_final.png')

...
def get_args():
    ...
    parser.add_argument("--rollout_hidden-dims", type=int, nargs='*', default=[256, 256, 256])
    ...
    parser.add_argument("--rollout-length-rollout-policy", type=int, default=1)
...
def train(args=get_args()):
    ...
    elif args.task == "halfcheetah-jump-hard":
        original_task = "halfcheetah" + args.task.lstrip(args.task.split('-')[0])
        env = gym.make("halfcheetah-random-v2")
        from gym.wrappers import RescaleAction
    ...
    if args.task.split('-')[0] == "hopper" or args.task.split('-')[0] == "walker2d":
    ...
    if args.rollout_policy_type == "optimistic":
        ...
        load_vae_path = f'vae_model-v2/{args.task[:-3]}'
        ...
        load_optimistic_policy_path = f'trained-optimistic-policy-v2/{args.task[:-3]}'

...
from ..dataset import (
    ...
    Transition,
    TransitionMiniBatch,
    TransitionRecord2rewards,
    TransitionRecord2rewardsMiniBatch,
...
class _Buffer(metaclass=ABCMeta):
    ...
    def to_mdp_dataset_with_next_observations(self) -> MDPDataset:
        """Convert replay data into static dataset.
        ...
        for transition in tail_transitions:
            ...
            for i, episode_transition in enumerate(episode_transitions):
                observations.append(episode_transition.observation)
                next_observations.append(episode_transition.next_observation)
                actions.append(episode_transition.action)
                rewards.append(episode_transition.reward)
                terminals.append(episode_transition.terminal)
                episode_terminals.append(i == len(episode_transitions) - 1)
...
class ReplayBufferRecord2rewards(ReplayBuffer):
    """Standard Replay Buffer.
    ...
    def __init__(
        ...
        self._prev_action = None
        self._prev_reward = 0.0
        self._prev_modified_reward = 0.0
        self._prev_terminal = 0.0
        self._prev_transition = None

    def append_with_modified_reward(
        self,
        observation: np.ndarray,
        action: np.ndarray,
        reward: float,
        ...
        if self._prev_observation is not None:
            ...
            transition = TransitionRecord2rewards(
                observation_shape=self._observation_shape,
                action_size=self._action_size,
                observation=self._prev_observation,
                action=self._prev_action,
                reward=self._prev_reward,
                modified_reward=self._prev_modified_reward,
                next_observation=observation,
                terminal=terminal,
                prev_transition=self._prev_transition,
            )
        ...
        self._prev_action = action
        self._prev_reward = reward
        self._prev_modified_reward = modified_reward
        self._prev_terminal = terminal
    ...
    def to_mdp_dataset_modified_reward(self) -> MDPDataset:
        """Convert replay data into static dataset.
        ...
        for transition in tail_transitions:
            ...
            for i, episode_transition in enumerate(episode_transitions):
                observations.append(episode_transition.observation)
                actions.append(episode_transition.action)
                rewards.append(episode_transition.modified_reward)
                terminals.append(episode_transition.terminal)
                episode_terminals.append(i == len(episode_transitions) - 1)
    ...
    def to_mdp_dataset_modified_reward_with_next_observations(self) -> [MDPDataset,dict]:
        """Convert replay data into static dataset.
    ...
    def sample(
        ...
        indices = np.random.choice(len(self._transitions), batch_size)
        transitions = [self._transitions[index] for index in indices]
        batch = TransitionRecord2rewardsMiniBatch(transitions, n_frames, n_steps, gamma)
        return batch

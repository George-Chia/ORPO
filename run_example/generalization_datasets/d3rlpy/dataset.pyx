...
from dataset cimport CTransition, CTransitionRecord2rewards
...
class MDPDataset:
    ...
    def reinitialize(self,
        ...
        num_episode = len(self._episodes)
        for _ in range(num_episode):
            episode = self._episodes.pop()
            del episode
        del self._episodes
        gc.collect()
...
ctypedef shared_ptr[CTransitionRecord2rewards] TransitionRecord2rewardsPtr
...
cdef class TransitionRecord2rewards:
    """ Transition class.
    ...
    """
    cdef TransitionRecord2rewardsPtr _thisptr
    cdef bool _is_image
    cdef bool _is_discrete
    cdef _observation
    cdef _action
    cdef _next_observation
    cdef TransitionRecord2rewards _prev_transition
    cdef TransitionRecord2rewards _next_transition
    ...
    def __cinit__(
        ...
        np.ndarray next_observation,
        float terminal,
        TransitionRecord2rewards prev_transition=None,
        TransitionRecord2rewards next_transition=None
    ):
        cdef TransitionRecord2rewardsPtr prev_ptr
        cdef TransitionRecord2rewardsPtr next_ptr
        ...
        self._thisptr = make_shared[CTransitionRecord2rewards]()
        self._thisptr.get().observation_shape = observation_shape
        self._thisptr.get().action_size = action_size
        self._thisptr.get().reward = reward
        self._thisptr.get().modified_reward = modified_reward
        self._thisptr.get().terminal = terminal
        self._thisptr.get().prev_transition = prev_ptr
        self._thisptr.get().next_transition = next_ptr
    ...
    cdef TransitionRecord2rewardsPtr get_ptr(self):
        return self._thisptr
    ...
    def modified_reward(self):
        """ Returns modified_reward at `t`.

        Returns:
            float: modified_reward at `t`.

        """
        return self._thisptr.get().modified_reward
    ...
    @prev_transition.setter
    def prev_transition(self, TransitionRecord2rewards transition):
        """ Sets transition to ``prev_transition``.
        ...
        """
        assert isinstance(transition, TransitionRecord2rewards)
        cdef TransitionRecord2rewardsPtr ptr
        ptr = transition.get_ptr()
        self._thisptr.get().prev_transition = ptr
        self._prev_transition = transition
    ...
    @next_transition.setter
    def next_transition(self, TransitionRecord2rewards transition):
        """ Sets transition to ``next_transition``.
    ...
    def clear_links(self):
        ...
        self._prev_transition = None
        self._next_transition = None
        self._thisptr.get().prev_transition = <TransitionRecord2rewardsPtr> nullptr
        self._thisptr.get().next_transition = <TransitionRecord2rewardsPtr> nullptr
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef void _stack_frames_Record2rewards(
    TransitionRecord2rewardsPtr transition,
    UINT8_t* stack,
    int n_frames,
    bool stack_next=False,
) nogil:
    ...
    # stack frames
    cdef TransitionRecord2rewardsPtr t = transition
    cdef int i, j
    cdef int head_channel
    cdef int tail_channel
    cdef int offset
...
cdef class TransitionRecord2rewardsMiniBatch:
    """ mini-batch of Transition objects.
    ...
    cdef np.ndarray _actions
    cdef np.ndarray _rewards
    cdef np.ndarray _modified_rewards
    cdef np.ndarray _next_observations
    cdef np.ndarray _terminals
    cdef np.ndarray _n_steps
    ...
    def __cinit__(
        ...
        self._actions = np.empty((size,) + action_shape, dtype=action_dtype)
        self._rewards = np.empty((size, 1), dtype=np.float32)
        self._modified_rewards = np.empty((size, 1), dtype=np.float32)
        self._next_observations = np.empty(
            (size,) + observation_shape, dtype=observation_dtype
        )
        self._terminals = np.empty((size, 1), dtype=np.float32)
        ...
        cdef void* actions_ptr = self._actions.data
        cdef FLOAT_t* rewards_ptr = <FLOAT_t*> self._rewards.data
        cdef FLOAT_t * modified_rewards_ptr = <FLOAT_t *> self._modified_rewards.data
        cdef void* next_observations_ptr = self._next_observations.data
        cdef FLOAT_t* terminals_ptr = <FLOAT_t*> self._terminals.data
        cdef FLOAT_t* n_steps_ptr = <FLOAT_t*> self._n_steps.data
        ...
        # get pointers to transitions
        cdef int i
        cdef TransitionRecord2rewards transition
        cdef vector[TransitionRecord2rewardsPtr] transition_ptrs
        for i in range(size):
            transition = transitions[i]
            transition_ptrs.push_back(transition.get_ptr())
        ...
        for i in prange(size, nogil=True):
            ...
            self._assign_to_batch(
                ...
                actions_ptr=actions_ptr,
                rewards_ptr=rewards_ptr,
                modified_rewards_ptr=modified_rewards_ptr,
                next_observations_ptr=next_observations_ptr,
                terminals_ptr=terminals_ptr,
                n_steps_ptr=n_steps_ptr,
                n_frames=n_frames,
    ...
    cdef void _assign_observation(
        self,
        int batch_index,
        TransitionRecord2rewardsPtr ptr,
        void* observations_ptr,
        int n_frames,
        bool is_image,
        bool is_next

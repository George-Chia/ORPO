...
def evaluate_on_environment_ForHatScore(
    env, fake_env, n_trials: int = 10, epsilon: float = 0.0, render: bool = False
) -> Callable[..., float]:
    """Returns scorer function of evaluation on environment.
        ...
        for _ in range(n_trials):
            ...
            while True:
                ...
                try:
                    ...
                    observation, _, done, _ , _, uncertainty,reward = fake_env.step(observation,action)
                except:
                    break
                step += 1
                ...
                if done or step>1000:
                    break
            episode_rewards.append(episode_reward)
            episode_uncertainty_list.append(episode_uncertainty)
        return float(np.mean(episode_rewards))

    return scorer


def evaluate_on_environment_ForUncertainty(
    env, fake_env, n_trials: int = 10, epsilon: float = 0.0, render: bool = False
) -> Callable[..., float]:
    """Returns scorer function of evaluation on environment.
    ...
    def scorer(algo: AlgoProtocol, *args: Any) -> float:
        ...
        for _ in range(n_trials):
            ...
            episode_rewards.append(episode_reward)
            episode_uncertainty_list.append(episode_uncertainty)
        return float(np.mean(episode_uncertainty_list))

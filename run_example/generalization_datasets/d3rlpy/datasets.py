...
def get_d4rl_part_mixreg(env_dataset, dataset_buffer, to_index=0) -> Tuple[MDPDataset, gym.Env]: # TODO to_index=?????????
    """Returns d4rl dataset and envrironment.
    ...
    try:
        ...
        to_index2 = dataset_buffer.size

        observations = np.concatenate((env_dataset.observations[:to_index,:], dataset_buffer.observations[:to_index2,:]),axis=0)
        actions = np.concatenate((env_dataset.actions[:to_index,:],dataset_buffer.actions[:to_index2,:]),axis=0)
        rewards = np.concatenate((env_dataset.rewards[:to_index],np.squeeze(dataset_buffer.rewards,axis=1)[:to_index2]),axis=0)
        terminals = np.concatenate((env_dataset.terminals[:to_index],np.squeeze(dataset_buffer.terminals,axis=1)[:to_index2]),axis=0)
        timeouts = np.concatenate((env_dataset.terminals[:to_index],np.squeeze(dataset_buffer.terminals,axis=1)[:to_index2]),axis=0)
        episode_terminals = np.logical_or(terminals, timeouts)
...
def load_buffer_first_time(dataset, dataset_buffer):
    to_index2 = dataset_buffer.size

    observations = dataset_buffer.observations[:to_index2,:]
    actions = dataset_buffer.actions[:to_index2,:]
    rewards = dataset_buffer.rewards[:to_index2]
    terminals = dataset_buffer.terminals[:to_index2]
    timeouts = dataset_buffer.terminals[:to_index2]
    episode_terminals = np.logical_or(terminals, timeouts)
    dataset.initialize(observations=np.array(observations, dtype=np.float32),
            actions=np.array(actions, dtype=np.float32),
            rewards=np.array(rewards, dtype=np.float32),
            terminals=np.array(terminals, dtype=np.float32),
            episode_terminals=np.array(episode_terminals, dtype=np.float32),)
...
def load_buffer(dataset, dataset_buffer):
    to_index2 = dataset_buffer.size
    ...
    timeouts = dataset_buffer.terminals[:to_index2]
    episode_terminals = np.logical_or(terminals, timeouts)
    dataset.reinitialize(observations=np.array(observations, dtype=np.float32),
            actions=np.array(actions, dtype=np.float32),
            rewards=np.array(rewards, dtype=np.float32),
            terminals=np.array(terminals, dtype=np.float32),
            episode_terminals=np.array(episode_terminals, dtype=np.float32),)

...
# class ScaledTanhNormalWrapper(torch.distributions.Normal):
#     def log_prob(self, action, raw_action=None):
#         if raw_action is None:
#             raw_action = self.arctanh(action)
#         log_prob = super().log_prob(raw_action).sum(-1, keepdim=True)
...
# class ScaledTanhDiagGaussian(DiagGaussian):
#     def __init__(
#         self,
#         latent_dim,
#         output_dim,
...
#             shape[1] = -1
#             sigma = (self.sigma_param.view(shape) + torch.zeros_like(mu)).exp()
#         return ScaledTanhNormalWrapper(mu, sigma,scale_times=self.scale_times)
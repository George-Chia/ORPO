...
class EnsembleDynamics(BaseDynamics):
    ...
    def step(
        ...
        elif self._uncertainty_mode == "ensemble_var":
            next_obses_mean = mean[..., :-1]
            penalty = next_obses_mean.var(0).mean(1)
        else:
            raise ValueError
        penalty = np.expand_dims(penalty, 1).astype(np.float32)
        assert penalty.shape == reward.shape
        info["uncertainty"] = penalty

        if self._penalty_coef:
                reward = reward - self._penalty_coef * penalty
                info["penalty_reward"] = reward

        if self._bonus_coef:
            # if self._uncertainty_mode == "aleatoric":
            #     bonus = np.amax(np.linalg.norm(std, axis=2), axis=0)
            # elif self._uncertainty_mode == "pairwise-diff":
            #     next_obses_mean = mean[..., :-1]
            #     next_obs_mean = np.mean(next_obses_mean, axis=0)
            #     diff = next_obses_mean - next_obs_mean
            #     bonus = np.amax(np.linalg.norm(diff, axis=2), axis=0)
            # elif self._uncertainty_mode == "ensemble_std":
            #     next_obses_mean = mean[..., :-1]
            #     bonus = np.sqrt(next_obses_mean.var(0).mean(1))
            # elif self._uncertainty_mode == "ensemble_var":
            #     next_obses_mean = mean[..., :-1]
            #     bonus = next_obses_mean.var(0).mean(1)
            # else:
            #     raise ValueError
            # bonus = np.expand_dims(bonus, 1).astype(np.float32)
            # assert bonus.shape == info["raw_reward"].shape
            # info["uncertainty"] = bonus
            bouns_reward = info["raw_reward"] + self._bonus_coef * info["uncertainty"]
            info["bouns_reward"] = bouns_reward

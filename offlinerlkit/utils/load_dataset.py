...
def get_dataset(h5path, next_observation_path=None):
    ...
    assert data_dict['terminals'].shape == (N_samples,), 'Terminals has wrong shape: %s' % (
        str(data_dict['rewards'].shape))
    if next_observation_path is not None:
        next_observations_array = np.load(next_observation_path)
        data_dict['next_observations'] = next_observations_array
    return data_dict

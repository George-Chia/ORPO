...
class OOMICQLPolicy(COMBOPolicy):
    ...
    # add bonus_rewards compared to COMBO
    def rollout(
            self,
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        real_batch, final_policy_fake_batch, rollout_policy_fake_batch = batch["real"], batch["final_policy_fake"], \
                                                                         batch["rollout_policy_fake"]
        mix_batch = {k: torch.cat([real_batch[k], final_policy_fake_batch[k], rollout_policy_fake_batch[k]], 0) for k in
                     real_batch.keys()}

        obss, actions, next_obss, rewards, terminals = mix_batch["observations"], mix_batch["actions"], \
                                                       mix_batch["next_observations"], mix_batch["rewards"], mix_batch[
                                                           "terminals"]
